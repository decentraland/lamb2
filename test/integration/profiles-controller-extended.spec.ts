...
test('integration tests for /profiles?erc721', function ({ components, stubComponents }) {
  ...
  it('calling with a single profile address, without nfts', async () => {
    ...
    const wearablesQuery =
      '{        P0x2: nfts(where: { owner: "0x2", searchItemType_in: ["wearable_v1", "wearable_v2", "smart_wearable_v1", "emote_v1"], urn_in: [] }, first: 1000) {        urn        }    }'
  ...
  it('calling with a single profile address, two eth wearables, one of them not owned (retrieved extended URNs from active entities)', async () => {
    ...
    content.fetchEntitiesByPointers
      .withArgs(addresses)
      .resolves(await Promise.all([profileEntityTwoEthWearablesExtended]))
  ...
  it('calling with a single profile address, two matic wearables, one of them not owned (retrieved extended URNs from active entities)', async () => {
    ...
    content.fetchEntitiesByPointers
      .withArgs(addresses)
      .resolves(await Promise.all([profileEntityTwoMaticWearablesExtended]))
    const wearablesQuery =
  ...
  it('calling with a single profile address, owning claimed name', async () => {
    ...
    const wearablesQuery =
      '{        P0x5: nfts(where: { owner: "0x5", searchItemType_in: ["wearable_v1", "wearable_v2", "smart_wearable_v1", "emote_v1"], urn_in: [] }, first: 1000) {        urn        }    }'
    theGraph.ethereumCollectionsSubgraph.query = sinon.stub().withArgs(wearablesQuery, {}).resolves({
      P0x5: []
    })
    theGraph.maticCollectionsSubgraph.query = sinon.stub().withArgs(wearablesQuery, {}).resolves({
  ...
  it('calling with a single profile address, two tpw wearables from same collection, one of them not owned', async () => {
    ...
    theGraph.ethereumCollectionsSubgraph.query = sinon.stub().withArgs(wearablesQuery, {}).resolves({ P0x6: [] })
  ...
  it('calling with a single profile address, five tpw wearables from two different collections, two of them not owned', async () => {
    ...
    theGraph.ethereumCollectionsSubgraph.query = sinon.stub().withArgs(wearablesQuery, {}).resolves({ P0x7: [] })
  ...
  it('calling with two profile addresses, owning everything claimed (retrieved extended URNs from active entities)', async () => {
    ...
    content.fetchEntitiesByPointers
      .withArgs(addresses)
      .resolves(await Promise.all([profileEntityFullBExtended, profileEntityFullAnotherExtended]))
    const wearablesQuery =
  ...
  it('calling with a single profile address with old body shape format', async () => {
    ...
    const wearablesQuery =
      '{        P0x9: nfts(where: { owner: "0x9", searchItemType_in: ["wearable_v1", "wearable_v2", "smart_wearable_v1", "emote_v1"], urn_in: [] }, first: 1000) {        urn        }    }'
    theGraph.ethereumCollectionsSubgraph.query = sinon.stub().withArgs(wearablesQuery, {}).resolves({ P0x9: [] })
  ...
  it('calling with a single profile address with snapshots referencing content', async () => {
    ...
    const wearablesQuery =
      '{        P0x10: nfts(where: { owner: "0x10", searchItemType_in: ["wearable_v1", "wearable_v2", "smart_wearable_v1", "emote_v1"], urn_in: [] }, first: 1000) {        urn        }    }'
    theGraph.ethereumCollectionsSubgraph.query = sinon.stub().withArgs(wearablesQuery, {}).resolves({ P0x10: [] })
