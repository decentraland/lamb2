...
describe('createSortingFromFilters', function () {
  const transferredAt = Date.now()
  const item1: OnChainWearable = {
    urn: 'urn-1',
    amount: 1,
  ...
  describe('when no orderBy is provided', () => {
    it('should default to rarest sorting (DESC)', () => {
      const sortFn = createSortingFromFilters()
      expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
    })
  })

  describe('when orderBy is rarity', () => {
    describe('and direction is DESC', () => {
      it('should sort by rarest first', () => {
        const sortFn = createSortingFromFilters('rarity', 'DESC')
        expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
      })
    })

    describe('and direction is ASC', () => {
      it('should sort by least rare first', () => {
        const sortFn = createSortingFromFilters('rarity', 'ASC')
        expect([item1, item2].sort(sortFn)).toEqual([item2, item1])
      })
    })

    describe('and direction is not provided', () => {
      it('should default to DESC', () => {
        const sortFn = createSortingFromFilters('rarity')
        expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
      })
    })
  })

  describe('when orderBy is name', () => {
    describe('and direction is ASC', () => {
      it('should sort alphabetically A-Z', () => {
        const sortFn = createSortingFromFilters('name', 'ASC')
        // "Covid Mask" comes before "Diamond earrings"
        expect([item1, item2].sort(sortFn)).toEqual([item2, item1])
      })
    })
    ...
    describe('and direction is DESC', () => {
      it('should sort alphabetically Z-A', () => {
        const sortFn = createSortingFromFilters('name', 'DESC')
        // "Diamond earrings" comes before "Covid Mask"
        expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
      })
    })
    ...
    describe('and direction is not provided', () => {
      it('should default to ASC', () => {
        const sortFn = createSortingFromFilters('name')
        expect([item1, item2].sort(sortFn)).toEqual([item2, item1])
      })
    })
  })

  describe('when orderBy is date', () => {
    describe('and direction is DESC', () => {
      it('should sort by newest first', () => {
        const sortFn = createSortingFromFilters('date', 'DESC')
        // item1 is newer (higher transferredAt)
        expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
      })
    })
    ...
    describe('and direction is ASC', () => {
      it('should sort by oldest first', () => {
        const sortFn = createSortingFromFilters('date', 'ASC')
        // item2 is older (lower transferredAt)
        expect([item1, item2].sort(sortFn)).toEqual([item2, item1])
      })
    })
    ...
    describe('and direction is not provided', () => {
      it('should default to DESC', () => {
        const sortFn = createSortingFromFilters('date')
        expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
      })
    })
  })

  describe('when orderBy is case insensitive', () => {
    it('should handle uppercase orderBy', () => {
      const sortFn = createSortingFromFilters('RARITY', 'DESC')
      expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
    })

    it('should handle mixed case orderBy', () => {
      const sortFn = createSortingFromFilters('RaRiTy', 'desc')
      expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
    })
  })

  describe('when orderBy is unknown', () => {
    it('should fallback to rarest DESC', () => {
      const sortFn = createSortingFromFilters('unknown', 'ASC')
      expect([item2, item1].sort(sortFn)).toEqual([item1, item2])
    })
  })
})
