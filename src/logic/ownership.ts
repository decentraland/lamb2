...
export async function ownedExtendedNFTsByAddress(
  components: Pick<AppComponents, 'theGraph' | 'config'>,
  nftIdsByAddressToCheck: Map<string, { urn: string; tokenId: string }[]>,
  querySubgraph: (theGraph: TheGraphComponent, nftsToCheck: [string, { urn: string; tokenId: string }[]][]) => any
): Promise<Map<string, { urn: string; tokenId: string }[]>> {
  // Check ownership for unknown nfts
