...
export async function createWearablesCachesComponent(components: Pick<AppComponents, 'config'>): Promise<WearablesCachesComponent> {
    ...
    const thirdPartyWearablesCache: LRU<string, wearableForResponse[]> = new LRU({ max: wearablesSize, ttl: wearablesAge})
